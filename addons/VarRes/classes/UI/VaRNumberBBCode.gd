...
func _get_configuration_warnings() -> PackedStringArray:
	if get_parent() is RichTextLabel:
		return []
	else:
		return ["Родительская нода должна быть RichTextLabel"]

func _ready() -> void:
	#print("ready VaRNumberBBCode!")
	assert(get_parent() is RichTextLabel, "VaRNumberBBCode parent in not RichTextLabel, %s"%[get_path()])
